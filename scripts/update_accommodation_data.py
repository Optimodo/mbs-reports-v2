...
def update_config_file(project_name, accommodation_data):
    """
    Update the project accommodation data file (in configs/accommodation_data/).
    
    Args:
        project_name: Name of the project
        accommodation_data: Structured accommodation data dictionary
    """
    # Create accommodation_data directory if it doesn't exist
    accom_data_dir = CONFIGS_DIR / 'accommodation_data'
    accom_data_dir.mkdir(exist_ok=True)
    
    # Path to the accommodation data file
    data_file = accom_data_dir / f"{project_name}.py"
    
    print(f"\n📝 Updating accommodation data file: {data_file}")
    
    # Generate ACCOMMODATION_DATA section
    data_section = f'''"""
Accommodation data for {project_name} project.
...
Last updated: {accommodation_data['last_updated']}
Source: {accommodation_data['source_file']}
"""

ACCOMMODATION_DATA = {{
    'total_apartments': {accommodation_data['total_apartments']},
    'last_updated': '{accommodation_data['last_updated']}',
    'source_file': '{accommodation_data['source_file']}',
...
'''
    ...
    # Write the accommodation data file
    with open(data_file, 'w', encoding='utf-8') as f:
        f.write(data_section)
    
    print(f"✓ Accommodation data file updated successfully")
    print(f"\n💡 Next steps:")
