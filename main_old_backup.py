    ...
    with DocumentDatabase() as db:
        ...
        for project_name in db_projects:
            ...
            try:
                ...
                # Generate certificate report if enabled for this project
                cert_settings = config.get('CERTIFICATE_SETTINGS', {})
                if cert_settings.get('enabled', False) and cert_settings.get('generate_report', False):
                    print(f"\nGenerating certificate report...")
                    
                    # Get certificate data by filtering from database
                    from utils.document_filters import filter_certificates, get_document_type_summary
                    from analyzers import get_counts
                    
                    # Get summary of certificates for debugging
                    doc_summary = get_document_type_summary(latest_data_df, config)
                    print(f"  Certificate documents: {doc_summary['certificates']}")
                    
                    if doc_summary['certificates'] > 0:
                        # Filter to only certificate documents
                        cert_data = filter_certificates(latest_data_df, config)
                        
                        if not cert_data.empty:
                            # Build certificate summary DataFrame by reprocessing snapshots with cert filter
                            cert_summary_rows = []
                            for idx, row in summary_df.iterrows():
                                snapshot_date = row['Date']
                                snapshot_time = row['Time']
                                ...
                                # Get snapshot docs and filter to certificates only
                                snapshot_docs = db.get_documents_for_snapshot(project_name, db_date, snapshot_time)
                                snapshot_certs = filter_certificates(snapshot_docs, config)
                                
                                if not snapshot_certs.empty:
                                    # Get counts for certificates
                                    cert_counts = get_counts(snapshot_certs, config, report_type='certificate')
                                    cert_counts['Date'] = snapshot_date
                                    cert_counts['Time'] = snapshot_time
                                    cert_summary_rows.append(cert_counts)
                            
                            if cert_summary_rows:
                                ...
                                if save_certificate_report_with_retry(cert_summary_df, cert_data, cert_output, config):
                                    print(f"  ✓ Certificate report saved: {cert_output}")
                                else:
                                    print(f"  ✗ Failed to save certificate report")
                            else:
                                print(f"  ℹ No certificate data in snapshots")
                        else:
                            print(f"  ℹ No certificate documents found in latest data")
                    else:
                        print(f"  ℹ No certificates configured or found for this project")
